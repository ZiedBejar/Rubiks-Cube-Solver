...
class WebcamApp:
    def __init__(self, window):
        ...
        label_instruction=tk.Label(window, text="The order of capture ", justify='left', bg=window['bg'], fg='#212E53', font=('Arial', 20, 'bold'))
        label_instruction.place(x=750, y=40)
        label_text = """
1) Capture the up face (with the white center cube) and the left face up (with the orange centre)


2) Capture the right face (with the white center cube) and the left face up (with the orange centre)


3) Capture the front face (with the white center cube) and the left face up (with the orange centre)


4) Capture the down face (with the white center cube) and the left face up (with the orange centre)


5) Capture the left face (with the white center cube) and the left face up (with the orange centre)


6) Capture the back face (with the white center cube) and the left face up (with the orange centre)
"""

        label = tk.Label(window, text=label_text, justify='left', bg=window['bg'], fg='#212E53', font=('Arial', 12, 'bold'))
        label.place(x=580, y=100)

        self.capture_button = tk.Button(window, text="Capture", command=self.capture_image, width=15,  bg="#FFCC80", font=('Arial', 12))
        self.capture_button.grid(row=1, column=0, padx=5, pady=5)

        self.solve_button = tk.Button(window, text="Solve Cube", command=self.solve_cube, width=15, state="disabled",  bg="#CA3C66", font=('Arial', 12))
        self.solve_button.grid(row=1, column=1, padx=5, pady=5)

        self.remove_last_button = tk.Button(window, text="Remove Last Matrix", command=self.remove_last_matrix, width=15, bg="#FFCC80", font=('Arial', 12))
        self.remove_last_button.grid(row=1, column=2, padx=5, pady=5)

        self.exit_button = tk.Button(window, text="Exit", command=self.exit_program, width=15, bg="#FFCC80", font=('Arial', 12))
        self.exit_button.grid(row=2, column=2, padx=5, pady=5)
        
        self.restart_button = tk.Button(window, text="Restart", command=self.restart, width=15, bg="#FFCC80", font=('Arial', 12))
    ...
    def update_webcam(self):
     ...
     if ret:
        ...
        text_color = (102, 60, 202)  # BGR equivalent of #CA3C66
        for i in range(3):
            for j in range(3):
    ...
    def remove_last_matrix(self):
        if len(self.captured_images) > 0:
            ...
            last_captured_image, last_captured_canvas = self.captured_images.pop()
            last_captured_canvas.destroy()
...
fenetre.iconbitmap('C:\\Users\\LENOVO\\Downloads\\logoenim.jpg')
fenetre['bg'] = '#B6D8F2'
